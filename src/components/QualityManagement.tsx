...
import { 
  Check, X, AlertTriangle, AlertCircle, RotateCw, RefreshCw, Loader2, ArrowRight, CornerDownRight, ShieldCheck, Search
} from "lucide-react";
...
export default function QualityManagement({ token, preFilter, onBack }: QualityManagementProps) {
  ...
  const [qualityStates, setQualityStates] = useState<Record<number, {
  ...
  const fetchBatches = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Eligible for quality updates (finished batches, not delivered)
        const active = data.filter((b: Batch) => b.stages?.finish === "completed" && b.stages?.delivered !== "completed");
        setBatches(active);

        // Prepopulate quality states
        const initialStates: typeof qualityStates = {};
        active.forEach((b: Batch) => {
          initialStates[b.id] = {
            status: b.quality_status || "pending",
            reason: b.quality_reason || "Quality Defect",
            custom_reason: b.quality_custom_reason || "",
            reworkType: "Dyeing Rework",
            reworkRemarks: "",
          };
        });
        setQualityStates(initialStates);
      }
    } catch (err) {
      console.error("Error loading batches for quality assessment:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveQuality = async (id: number) => {
    const state = qualityStates[id];
    if (!state) return;
    ...
    try {
      const res = await fetch("/api/quality/update", {
        ...
        body: JSON.stringify({
          ...
          quality_reason: state.status === "not_ok" ? state.reason : "",
          quality_custom_reason: state.status === "not_ok" ? state.custom_reason : "",
        }),
      });
      ...
      if (res.ok) {
        ...
        alert(err.error || "Failed to update quality assessment.");
      }
    } catch (err) {
    ...
    try {
      ...
      const res = await fetch("/api/rework/update", {
        ...
        body: JSON.stringify({
          batch_id: id,
          type: mappedType,
          remarks: state.reworkRemarks || `Quality Control Rejection Rework - ${state.reason}`,
          reason: state.reason || "Quality Defect",
  ...
  const filteredBatches = batches.filter((b) => {
    const currentStatus = b.quality_status || "pending";
    ...
    <div className="space-y-6">
      ...
      <section className="flex border-b border-slate-200 dark:border-slate-800 gap-2 overflow-x-auto no-print">
        {(["all", "pending", "ok", "not_ok"] as const).map((tab) => {
          const labels = { all: "All Batches", pending: "Pending Quality", ok: "Quality PASSED (OK)", not_ok: "Quality REJECTED (NOT OK)" };
      ...
      {loading ? (
              ...
              >
                <div className="space-y-2.5">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    ...
                    Color: <strong className="text-slate-700 dark:text-slate-300">{batch.color}</strong>
                  </p>
                </div>
                  ...
                  <div>
                    {batch.quality_status === "ok" ? (
                      <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md border border-emerald-100 dark:border-emerald-900/30">
                        <Check className="w-3.5 h-3.5" /> OK Passed
                      </span>
                    ) : batch.quality_status === "not_ok" ? (
        ...
        <div className="text-center p-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400">
          No batches found matching the chosen quality category.
        </div>
      )}

      {/* Pop-up Modal for Quality Inspection */}
      {selectedBatchId !== null && (() => {
        const batch = batches.find(b => b.id === selectedBatchId);
        if (!batch) return null;

        const state = qualityStates[batch.id] || { status: "pending", reason: "Quality Defect", custom_reason: "" };
            ...
            >
              ...
              <div className="p-6 space-y-6">
                <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-800">
                  ...
                  {batch.quality_status === "ok" ? (
                    <span className="text-emerald-500 font-bold">OK Passed</span>
                  ) : batch.quality_status === "not_ok" ? (
                    <span className="text-rose-500 font-bold">Defective (NOT OK)</span>
                  ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                  ...
                  <div className="md:col-span-5 space-y-4">
                    ...
                    <button
                      disabled={isSaving}
                      onClick={() => handleSaveQuality(batch.id)}
                    ...
                    {state.status === "not_ok" ? (
                      <div className="p-4 rounded-xl border border-rose-100 dark:border-rose-900/30 bg-rose-50/20 dark:bg-rose-950/10 space-y-3.5">
                        <div className="flex gap-2 items-center text-rose-600 dark:text-rose-400 font-bold text-xs font-mono uppercase">
                          ...
                          <span>Defective Quality Diagnostics</span>
                        </div>

                        {/* Custom explanation */}
                        <div className="space-y-1">
                          <label className="text-xs text-slate-500 font-semibold block">Custom Defect Notes</label>
                          <input
                            type="text"
                            placeholder="Type additional specific quality defects..."
                        ...
                        {/* Dynamic Reprocess Trigger section inside quality not ok */}
                      ...
                      <div className="p-12 text-center text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-950/20">
                        ...
                        <span className="text-xs font-medium">Select "NOT OK (Fail)" to log defect types or trigger immediate batch recycle.</span>
                      </div>
                    )}
                  </div>
                </div>
